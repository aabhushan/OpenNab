...
class Plugin_reboot extends Plugin
{
	
	function Plugin_reboot()
	{
	}
	...
	{
		if( $this->GetData($burrow,'reboot') === false )
			return false;
		$pingRequest->Reboot();
		addToLog( 'reboot: now', 3 );
		$this->ClearData($burrow,'reboot');
		return true;
	}
	...
	{
		if( !array_key_exists('reboot',$parameters) )
			return false;
		$this->SetData($burrow,'reboot',true);
		$reply->Add( 'Reboot', 'ok' );
		return true;
	}
